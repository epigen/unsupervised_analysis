...
def iterative_classification(data, labels, n_trees=100, max_iterations=100):
    ...
    for iteration in range(max_iterations):
        ...
        merge_indices = np.unravel_index(
            np.argmax(cm_symmetric, axis=None), cm_symmetric.shape
        )
        labels = np.array(
            [merge_indices[0] if num == merge_indices[1] else num for num in labels]
        )
...
# run clustification
clustering_new = iterative_classification(
    data.to_numpy(), clustering_init  # , n_trees=100, max_iterations=100
)

# save clustering as CSV
pd.DataFrame({"clustification_clustering": clustering_new}, index=data.index).to_csv(
