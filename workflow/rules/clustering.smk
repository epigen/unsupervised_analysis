...
rule leiden_cluster:
    ...
    output:
        clustering=os.path.join(
            ...
            "{sample}",
            "Leiden",
            "Leiden_{metric}_{n_neighbors}_{partition_type}_{resolution}_clustering.csv",
        ),
    resources:
    ...
    log:
        os.path.join(
            "logs",
            "rules",
            "leiden_{sample}_{metric}_{n_neighbors}_{partition_type}_{resolution}_clustering.log",
        ),
    params:
...
rule aggregate_clustering_results:
    ...
    output:
        aggregated_clusterings=os.path.join(
            config["result_path"],
            "unsupervised_analysis",
            "{sample}",
            "{method}",
            "{method}_clusterings.csv",
        ),
    resources:
        mem_mb=config.get("mem", "16000"),
    log:
        os.path.join(
            "logs", "rules", "aggregate_clustering_results_{sample}_{method}.log"
        ),
    run:
...
rule aggregate_all_clustering_results:
    ...
    output:
        metadata_clusterings=os.path.join(
            config["result_path"],
            "unsupervised_analysis",
            "{sample}",
            "metadata_clusterings.csv",
        ),
    resources:
        mem_mb=config.get("mem", "16000"),
    log:
