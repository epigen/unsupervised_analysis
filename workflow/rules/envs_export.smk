...
rule env_export:
    output:
        report(
            os.path.join(result_path, "envs", "{env}.yaml"),
...
rule config_export:
    output:
        configs=report(
            os.path.join(
                result_path, "configs", "{}_config.yaml".format(config["project_name"])
            ),
...
rule annot_export:
    ...
    output:
        annot=report(
            os.path.join(
                result_path, "configs", "{}_annot.csv".format(config["project_name"])
            ),
            caption="../report/configs.rst",
            category="Configuration",
            subcategory="{}_{}".format(config["project_name"], module_name),
