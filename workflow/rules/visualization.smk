...
rule plot_dimred_features:
        ...
        plot=report(
            directory(
                os.path.join(
                    ...
                    "{method}",
                    "plots",
                    "{method}_{parameters}_{n_components}",
                    "features",
                )
            ),
    ...
    log:
        os.path.join(
            "logs",
            "rules",
            "plot_features_{sample}_{method}_{parameters}_{n_components}.log",
        ),
    params:
...
rule plot_dimred_metadata:
    ...
    log:
        os.path.join(
            "logs",
            "rules",
            "plot_metadata_{sample}_{method}_{parameters}_{n_components}.log",
        ),
    params:
        size=config["scatterplot2d"]["size"],
        alpha=config["scatterplot2d"]["alpha"],
...
rule plot_pca_diagnostics:
    ...
    log:
        os.path.join(
            "logs", "rules", "plot_{method}_diagnostics_{sample}_{parameters}.log"
        ),
    script:
        "../scripts/plot_pca.R"
...
rule plot_umap_diagnostics:
    input:
        umap_object=os.path.join(
            result_path, "{sample}", "{method}", "{method}_{parameters}_object.pickle"
        ),
    output:
        diagnostics_plot=report(
            os.path.join(
    ...
    log:
        os.path.join(
            "logs", "rules", "plot_diagnostics_{sample}_{method}_{parameters}.log"
        ),
    script:
...
rule plot_umap_connectivity:
    ...
    log:
        os.path.join(
            "logs", "rules", "plot_connectivity_{sample}_{method}_{parameters}.log"
        ),
    script:
...
rule plot_dimred_interactive:
    ...
    log:
        os.path.join(
            "logs",
            "rules",
            "plot_interactive_{sample}_{method}_{parameters}_{n_components}.log",
        ),
    params:
...
rule plot_dimred_clustering:
    ...
    log:
        os.path.join(
            "logs",
            "rules",
            "plot_clustering_{sample}_{method}_{parameters}_{n_components}.log",
        ),
    params:
        size=config["scatterplot2d"]["size"],
        alpha=config["scatterplot2d"]["alpha"],
