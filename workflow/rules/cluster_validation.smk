    ...
    output:
        expand(
            os.path.join(
                ...
                "{{sample}}",
                "cluster_validation",
                "external_index_{index}.csv",
            ),
            index=indices_external,
        ),
    resources:
        mem_mb=config.get("mem", "16000"),
    threads: config.get("threads", 1)
...
rule validation_internal:
    ...
    output:
        internal_indices=os.path.join(
            config["result_path"],
            "unsupervised_analysis",
            "{sample}",
            "cluster_validation",
            "internal_index_{internal_index}.csv",
        ),
    resources:
    ...
    log:
        os.path.join(
            "logs", "rules", "validation_internal_{internal_index}_{sample}.log"
        ),
    params:
    ...
    input:
        expand(
            os.path.join(
                ...
                "{{sample}}",
                "cluster_validation",
                "internal_index_{index}.csv",
            ),
            index=indices_internal,
        ),
    output:
        internal_indices_ranked=os.path.join(
            config["result_path"],
            "unsupervised_analysis",
            "{sample}",
            "cluster_validation",
