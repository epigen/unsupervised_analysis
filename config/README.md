...
- workflow config schema (`config/config.schema.yaml`): JSON Schema used by Snakemake to validate the workflow configuration before execution.
- sample annotation (annotation): CSV file consisting of four mandatory columns.
- annotation schema (`config/annotation.schema.yaml`): JSON Schema used by Snakemake to validate each annotation row before execution.
